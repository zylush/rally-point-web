...
import { createPauseRuntime, preparePauseRunner, verifyPauseRunner, pauseCliEnvironment, runPauseCommand, executePauseCli } from '../scripts/reservation-pause-staging-runtime.mjs'
...
function fixture() {
  const parent = mkdtempSync(join(root, 'docs/release-private/pause-runtime-test-'))
  folders.push(parent)
  return { root, packet: join(parent, 'packet') }
}
afterEach(() => {
  for (const folder of folders.splice(0)) {
    if (!folder.startsWith(join(root, 'docs/release-private/pause-runtime-test-'))) throw new Error('Unsafe test cleanup')
...
describe.sequential('staging pause filesystem/CLI adapter (fake CLI only)', () => {
  ...
  it('runs the whole approved orchestration with a fake CLI and real catalog/grant postflight', async () => {
    ...
    run.mockImplementation(async (program, args) => {
      if (args[1] === 'query') {
        ...
        return { status: 0, stdout: JSON.stringify({ rows: (applied ? after : before)[name] }), stderr: '' }
      }
      if (args[1] === 'push') {
