...
import pg from 'pg'
import { databases, manifestSha256, localConfig, verifyContainer, verifyDockerEndpoint, createFresh,
  parseTap, executeOnce } from './reservation-resume-executor-guards.mjs'
...
const evidence = join(root, 'docs/release-private/reservation-resume-gate23-20260926-r1')
...
function docker(args, input = '') {
  const out = spawnSync(dockerExe, args, { input, encoding: 'utf8', shell: false,
    timeout: 120000, maxBuffer: 16 * 1024 * 1024, stdio: ['pipe','pipe','pipe'] })
  if (out.status !== 0) {
    const text = String(out.stderr || out.error || 'Docker command failed')
    const name = `failure-${String(phase).replaceAll(/[^a-z0-9-]/g,'-')}.private.txt`
    if (!existsSync(join(evidence,name))) save(name, text.slice(0, 8192))
...
async function identity(client, database, addresses) {
  ...
  assert.ok(addresses.includes(row.address), 'Connection is not the approved local container')
...
async function main() {
  ...
  save('ATTEMPT.json',{kind:'one-shot-disposable-local-rehearsal',manifestSha256,databases,
    ...
    guardsSha256:sha(readFileSync(join(root,'scripts/reservation-resume-executor-guards.mjs'))),
  ...
  await executeOnce(true,()=>{},async()=>{
    ...
    verifyDockerEndpoint(JSON.parse(docker(['context','inspect'])))
    const inspected=JSON.parse(docker(['inspect','supabase_db_rally-point-web']))
    const local=verifyContainer(inspected)
    password=local.password
    record({phase:'docker-identity',image:inspected[0].Config.Image})
    ...
    await admin.connect()
    try {
      await identity(admin,'postgres',local.addresses)
      ...
      await createFresh((sql,params)=>admin.query(sql,params),async database=>{
        ...
        try {
          await identity(client,database,local.addresses)
